function showAuthAlert(title, message, type = "success") {
  ...
  toast.className = `
      pointer-events-auto w-fit max-w-[90vw] md:max-w-xs
      rounded-xl border ${t.border} ${t.bg} p-3 shadow-lg
      opacity-0
      transform
      -translate-x-10 md:-translate-x-12 md:-translate-x-14
  ...
  toast.innerHTML = `
      <div class="flex items-start gap-3">
        <div class="${t.text} mt-0.5">${t.icon}</div>
        <div>
  ...
  requestAnimationFrame(() => {
    toast.classList.remove("opacity-0", "-translate-x-10", "-translate-y-4");
    toast.classList.add("opacity-100", "translate-x-0", "translate-y-0");
  });

    // Auto remove animation after 6s
  setTimeout(() => {
    toast.classList.remove("opacity-100", "translate-x-0", "translate-y-0");
    toast.classList.add("opacity-0", "-translate-x-10", "-translate-y-4");
