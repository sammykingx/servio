...
class PolicyFailure:
    ...
    EMAIL_NOT_VERIFIED = FailureDetail(
        "EMAIL_VERIFICATION_REQUIRED", "Email Verification Pending"
    )
    PROJECT_NOT_PUBLISHED = FailureDetail("GIG_NOT_PUBLISHED", "Project Unavailable")
    PROJECT_START_DATE_PASSED = FailureDetail(
        "GIG_START_DATE_PASSED", "Application Window Closed"
    )
    ...
    NOT_QUALIFIED_FOR_ROLES = FailureDetail(
        "NOT_QUALIFIED_FOR_ROLES", "Requirement Mismatch"
    )
    ...
    INVALID_ACTION = FailureDetail(
        "INVALID_ACTION", "Action Restricted"
    )
    PROPOSAL_FINALIZED = FailureDetail(
    "PROPOSAL_FINALIZED", "Proposal Already Finalized"
    )
...
class ValidationFailure:
    ...
    DURATION_EXCEEDS_LIMIT = FailureDetail(
        "DURATION_EXCEEDS_LIMIT", "Duration Too Long"
    )
    INVALID_INDUSTRY = FailureDetail("INVALID_INDUSTRY", "Invalid Industry Selected")
    MULTIPLE_INDUSTRIES_NOT_ALLOWED = FailureDetail(
        "MULTIPLE_INDUSTRIES_NOT_ALLOWED", "Single Industry Required"
    )
