...
class PaymentService:
    ...
    def process_payment(self, reference: str) -> GatewayInitResponse:
        try:
            ...
            return self._handle_policy_violation(payment_entity, err, phase=PaymentPhase.INITIALIZATION)

    def verify(self, reference: str) -> GatewayVerifyResponse:
        """
            Coordinates payment verification between the gateway and internal domain state.
        """
        try:
    ...
    # def _handle_policy_violation(self, payment: PaymentEntity, err: PolicyViolationError,*, phase:PaymentPhase):
    #     """Special handling for terminal policy states like 'Already Processed'."""
    #     response_class_map = {
    #         PaymentPhase.INITIALIZATION: GatewayInitializationResultEntity,
    #         PaymentPhase.VERIFICATION: GatewayVerificationResultEntity
    #     }
    #     response_class = response_class_map.get(phase)
        
    #     if err.code == PaymentFailure.ALREADY_PROCESSED.code:
    #         return response_class(
    #             gateway=payment.gateway,
    #             message=payment.gateway_reference,
    #             is_successful=False,
